...
async function ensureOwnerProfile(userId: string, nome: string): Promise<void> {
  ...
  if (profile?.role !== OWNER_ROLE) {
    throw new Error('Conta criada, mas o papel de CEO não foi aplicado. Contate o suporte.');
  }
}
...
async function fetchProfileByUserId(userId: string): Promise<ProfileRow | null> {
  ...
  if (avatarData && 'avatar_url' in avatarData) {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(
    async (credentials: LoginCredentials): Promise<AuthError | null> => {
    ...
    async (credentials: RegisterCredentials): Promise<RegisterResult> => {
      ...
      try {
        ...
        const { data, error } = await supabase.auth.signUp({
          email,
          password: credentials.password,
          options: { data: { nome, role: OWNER_ROLE } },
        });

        if (error) {
          return {
            status: 'error',
            error: mapAuthError(error as unknown as Parameters<typeof mapAuthError>[0]),
          };
        }
  ...
  const beginRegistration = useCallback((credentials: RegisterCredentials): AuthError | null => {
    const validationError = validateRegister(credentials);
    if (validationError) {
      return validationError;
    }
  ...
  const completeRegistration = useCallback(async (): Promise<RegisterResult> => {
    if (!pendingRegistration) {
      return { status: 'error', error: { message: 'Nenhum cadastro pendente encontrado.' } };
    }
