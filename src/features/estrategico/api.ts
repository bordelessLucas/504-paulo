...
export type ColaboradorAnualDetalhe = {
...
export function filtrarHistoricoPorAno(
  items: AvaliacaoHistoricoItem[],
  anoReferencia: number,
): AvaliacaoHistoricoItem[] {
  return items.filter((item) => new Date(item.dataReferencia).getFullYear() === anoReferencia);
}
...
export async function fetchColaboradorAnualDetalhe(
  colaboradorId: string,
  anoReferencia = new Date().getFullYear(),
): Promise<ColaboradorAnualDetalhe> {
  const [medias, decisaoExistente, historico] = await Promise.all([
    fetchMediasAnuaisColaborador(colaboradorId, anoReferencia),
    fetchDecisaoAnualExistente(colaboradorId, anoReferencia),
    fetchHistoricoAvaliacoesCompleto(colaboradorId),
  ]);
